...
function Move({ moveData }) {
  ...
  const onPageChange = (event) => {
    ...
    const take = targetEvent.value === "All" ? moveData.length : event.page.take; // All인 경우 : data의 길이를 전부 보여줌, 아닌 경우 : 선택한 숫자만큼 보여줌
  ...
  const sortedData = orderBy(moveData, sort);
    ...
    <div>
      ...
      >
        <Grid
          ...
          total={moveData.length} // 전체 데이터 항목 수 (그리드가 전체 데이터의 크기를 알고 페이징 처리를 할 수 있음)
          pageable={{
